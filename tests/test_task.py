...
def add_tasks(db_session):
    # Every test will just append so delete every time and create new
    db_session.query(Task).delete()
    db_session.commit()

    task1 = Task(todo="First Todo", description="First Todo's Description")
    db_session.add(task1)

    task2 = Task(todo="Second Todo", description="Second Todo's Description")
